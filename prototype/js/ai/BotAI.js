 ...
 * Manages tactical decision making, jungler macro, combo chaining, survival spells, laning, and adaptive shopping
 */
class BotAI {
  constructor(hero) {
    this.hero = hero;
  ...
  update(dt, game) {
    ...
    const floatText = game.addFloatingText ? game.addFloatingText.bind(game) : (game.hud && game.hud.addFloatingText ? game.hud.addFloatingText.bind(game.hud) : () => {});
    const clickWave = game.createClickWave ? game.createClickWave.bind(game) : () => {};
    const walls = (typeof ROCK_WALLS !== 'undefined' ? ROCK_WALLS : window.ROCK_WALLS) || [];
    ...
    this.decisionTimer = 0.20; // 5 decisions per second
