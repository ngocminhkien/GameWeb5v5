/**
 * OOP HUD Manager (Prototype)
 * Encapsulates Scoreboard, Match Clock, Progression Bar, Gold Counter,
 * Upgrade Buttons, Skill Cooldowns, Announcements, Floating Text Particles,
 * and Item Shop & Inventory System.
 */
class HUD {
  constructor() {
  ...
  selectShopItem(itemId, player) {
    ...
    if (buildsIntoContainer) {
      ...
      const buildsInto = Object.values(catalog).filter(other => other.recipe && other.recipe.includes(itemId));
  ...
  updateBuyButton(player) {
    ...
    const catalog = window.ITEMS || {};
    const it = catalog[this.selectedItemId];
    if (!it || !player) {
  ...
  updateChampionSkills(champId) {
    const catalog = window.CHAMPIONS || {};
    const champ = catalog[champId];
    if (!champ) return;
  ...
  renderChampCards() {
    ...
    const catalog = window.CHAMPIONS || {};
    Object.values(catalog).forEach(champ => {
      const card = document.createElement('div');
  ...
  updateChampSelectedInfo() {
    const catalog = window.CHAMPIONS || {};
    const champ = catalog[this.selectedChampId];
    if (!champ) return;
