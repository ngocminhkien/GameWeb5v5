...
export class Monster extends Entity {
  ...
  update(dt, createClickWave) {
    ...
    if (dBack > 8) {
      ...
      this.facingAngle = Math.atan2(dy, dx);
      this.x += (dx / dBack) * this.speed * dt;
      this.y += (dy / dBack) * this.speed * dt;
  ...
  draw(ctx, camera) {
    ...
    } else if (this.id.includes('blue_buff')) {
      this.drawBlueBuff(ctx, sx, sy);
    } else if (this.id.includes('red_buff')) {
      this.drawRedBuff(ctx, sx, sy);
    } else if (this.id.includes('wolves')) {
      this.drawWolves(ctx, sx, sy);
    } else if (this.id.includes('raptors')) {
      this.drawRaptors(ctx, sx, sy);
    } else if (this.id.includes('gromp')) {
