...
export class Hero extends Entity {
  constructor(config = {}) {
    ...
    const cDef = CHAMPIONS[this.championId] || CHAMPIONS.mage;
    ...
    this.maxHp = config.hp || cDef.baseStats.hp;
    this.hp = this.maxHp;
    this.maxMana = config.mana || cDef.baseStats.mana;
    ...
    this.tempestStacks = 0; // Yasuo Q
    this.rangersFocusTimer = 0; // Ashe W
    this.thunderclapTimer = 0; // Malphite W
    this.graniteTimer = 0; // Malphite Passive
    this.asDebuffTimer = 0;
    this.attackSpeedDebuff = 0;
    this.spawnWindWall = null; // Yasuo W
    this.garenQTimer = 0; // Garen Q speed & silence on next hit
    this.garenWTimer = 0; // Garen W 40% damage reduction
    this.garenSpinTimer = 0; // Garen E Judgment spin
    this.garenSpinTickTimer = 0;
    this.outOfCombatTimer = 0; // Garen Passive out-of-combat regen
    this.cosmicZone = null; // Lumina R cosmic silence field
  ...
  setChampion(champId) {
    ...
    this.baseMaxHp = cDef.baseStats.hp + (this.level - 1) * CONSTANTS.STAT_GROWTH.hp;
    this.baseMaxMana = cDef.baseStats.mana + (this.level - 1) * CONSTANTS.STAT_GROWTH.mana;
    this.baseAttackDamage = cDef.baseStats.attackDamage + (this.level - 1) * CONSTANTS.STAT_GROWTH.attackDamage;
    this.baseHpRegen = cDef.baseStats.hpRegen + (this.level - 1) * CONSTANTS.STAT_GROWTH.hpRegen;
    this.baseManaRegen = cDef.baseStats.manaRegen + (this.level - 1) * CONSTANTS.STAT_GROWTH.manaRegen;
    this.baseSpeed = cDef.baseStats.speed;
    this.baseAttackSpeed = cDef.baseStats.attackSpeed;
    this.attackRange = cDef.baseStats.attackRange;
    this.baseAttackRange = cDef.baseStats.attackRange;
  ...
  addExp(amount, onLevelUp = null, addFloatingText = null) {
    ...
    while (this.level < CONSTANTS.MAX_LEVEL && this.exp >= this.maxExp) {
      ...
      this.baseAttackDamage += growth.attackDamage;
      this.baseHpRegen += growth.hpRegen;
      this.baseManaRegen += growth.manaRegen;
    ...
    function matchRequirement(reqId) {
      for (let s of availableSlots) {
        if (!s.used && s.id === reqId) {
          s.used = true;
          return [{ slotIdx: s.slotIdx, id: reqId, cost: (ITEMS[reqId] ? ITEMS[reqId].cost : 0) }];
        }
      }
      const reqDef = ITEMS[reqId];
  ...
  takeDamage(amount, source = null, addFloatingText = null) {
    ...
    // Phản đòn Giáp Gai (Thornmail / Brambles Vest)
  ...
  update(dt, rockWalls, bushes, projectiles = null) {
    ...
    // Trạng thái Ngưng Đọng (Đồng Hồ Cát Zhonya)
    if (this.isStasis) {
      this.stasisTimer -= dt;
  ...
  fireBasicAttack(target, projectiles) {
    if (!target || !target.alive || this.isStasis || this.isReviving) return;
  ...
  castSkillshot(targetX, targetY, projectiles, addFloatingText = null) {
    ...
    const cfg = this.championDef.skills.Q;
    ...
    if (this.championId === 'mage') {
      ...
      // Lumina Star Orb: Fires exploding star sphere that stuns on impact
      const damage = Math.round(cfg.damage[rankIdx] + (this.abilityPower || 0) * 0.60);
      projectiles.push(new Projectile({
        x: this.x + Math.cos(angle) * (this.radius + 10),
        y: this.y + Math.sin(angle) * (this.radius + 10),
      ...
      // Lumina Passive: +25% movement speed for 2.5s
      ...
      // Garen Decisive Strike: Cleanse slows, 40% speed boost for 3s, empowers next basic attack with silence
      this.slowRatio = 0;
      this.slowTimer = 0;
  ...
  castShield(addFloatingText = null, allHeroes = []) {
    ...
    const cfg = this.championDef.skills.W;
    const manaCost = cfg.mana[rankIdx];
    const cd = cfg.cd[rankIdx] * (1 - (this.cooldownReduction || 0));
    ...
    if (this.championId === 'mage') {
      const shield = Math.round((cfg.shield[rankIdx] + (this.abilityPower || 0) * 0.60) * (this.hasSpiritVisage ? 1.25 : 1.0));
      this.shield += shield;
      ...
      // Garen W: Courage - Shield + 40% damage reduction for 3.5s
  ...
  castDash(targetX, targetY, rockWalls, addFloatingText = null, enemyHeroes = [], allyHeroes = []) {
    ...
    const cfg = this.championDef.skills.E;
    const manaCost = cfg.mana[rankIdx];
    const cd = cfg.cd[rankIdx] * (1 - (this.cooldownReduction || 0));
    ...
    if (this.championId === 'mage') {
      ...
      const damage = Math.round(cfg.damage[rankIdx] + this.attackDamage * 0.40);
      for (let e of enemyHeroes) {
        if (e.alive && e.team !== this.team && this.distanceTo(e) <= 150) {
      ...
      const damage = Math.round(cfg.damage[rankIdx] + this.armor * 0.40);
      const radius = cfg.range || 350;
      for (let e of enemyHeroes) {
        if (e.alive && e.team !== this.team && this.distanceTo(e) <= radius) {
          e.takeDamage(damage, this, addFloatingText);
          e.asDebuffTimer = 3.0;
      ...
      // Lumina E: Fountain of Life - Heal self & lowest HP percentage ally within 450px
      const healAmt = Math.round((cfg.heal[rankIdx] + (this.abilityPower || 0) * 0.55) * (this.hasSpiritVisage ? 1.25 : 1.0));
      this.hp = Math.min(this.maxHp, this.hp + healAmt);
      ...
      for (let a of (allyHeroes || [])) {
        if (a.alive && a.team === this.team && a !== this && this.distanceTo(a) <= (cfg.range || 450)) {
      ...
      // Garen E: Judgment Spin - starts 3s spinning blades
  ...
  castUltimate(heroes, towers, monsters, createClickWave = null, addFloatingText = null, onKill = null, projectiles = null) {
    ...
    const cfg = this.championDef.skills.R;
    const manaCost = cfg.mana[rankIdx];
    const cd = cfg.cd[rankIdx] * (1 - (this.cooldownReduction || 0));
    ...
    if (this.championId === 'assassin') {
      ...
      let minD = 1200;
      for (let h of heroes) {
        if (h.alive && h.team !== this.team && (h.knockupTimer > 0 || h.stunTimer > 0)) {
      ...
      for (let h of heroes) {
        if (h.alive && h.team !== this.team && Math.hypot(h.x - airborneEnemy.x, h.y - airborneEnemy.y) <= 260) {
          h.knockupTimer = 1.5;
    ...
    if (this.championId === 'fighter') {
      // Garen Demacian Justice: Targeted execution on lowest enemy hero in 500px
      let bestTarget = null;
      let lowestHp = 999999;
      for (let h of heroes) {
        if (h.alive && h.team !== this.team && this.distanceTo(h) <= (cfg.range || 500)) {
          if (h.hp < lowestHp) {
            lowestHp = h.hp;
            bestTarget = h;
          }
    ...
    if (this.championId === 'mage') {
      ...
      for (let tw of towers) {
        if (tw.team !== this.team && tw.alive && this.distanceTo(tw) <= radius) {
          ...
          tw.takeDamage(Math.max(10, damage - 60));
      ...
      for (let h of heroes) {
        if (h.team !== this.team && h.alive && this.distanceTo(h) <= (cfg.radius || 650)) {
          h.takeDamage(damage, this, addFloatingText);
          h.silenceTimer = 1.5;
  ...
  recall(addFloatingText = null) {
    if (this.cooldowns.B > 0 || this.stunTimer > 0 || this.knockupTimer > 0) return;
    this.cooldowns.B = this.maxCooldowns.B;

    // Empowered recall with Baron buff (2s instead of 4s!)
    const recallDuration = (this.hasBaronBuff && this.baronTimer > 0) ? 2.0 : 4.0;
    const txt = (this.hasBaronBuff && this.baronTimer > 0) ? '👑 Biến về Siêu Tốc Baron (2s)...' : '✨ Đang biến về...';
    if (addFloatingText) addFloatingText(txt, this.x, this.y - 35, '#79c0ff');
  ...
  draw(ctx, camera, mouse) {
    ...
    // Ground shadow & vertical offset if Knocked Up
