...
/**
 * TERRAIN CONFIGURATION - BASED ON LEAGUE OF LEGENDS, DOTA 2 & ARENA OF VALOR
 * Size: 15000 x 15000 px, 100% Symmetrical through Center (7500, 7500)
 */

// 1. DÒNG SÔNG CHỮ S TỰ NHIÊN (REALISTIC S-CURVED RIVER)
// Bắt đầu từ ngã 3 Top (2600, 1200) uốn lượn qua Hang Baron, cắt ngang Mid (7500, 7500),
// uốn lượn qua Hang Rồng và đổ ra ngã 3 Bot (13800, 12400)
...
// 2. CẤU TRÚC 3 ĐƯỜNG CHÍNH (3 LANES)
export const LANES = {
  mid: {
    from: { x: CONSTANTS.BLUE_BASE.x, y: CONSTANTS.BLUE_BASE.y },
    to: { x: CONSTANTS.RED_BASE.x, y: CONSTANTS.RED_BASE.y },
    center: { x: CONSTANTS.CENTER_X, y: CONSTANTS.CENTER_Y }
  },
  top: {
    points: [
      { x: CONSTANTS.BLUE_BASE.x, y: CONSTANTS.BLUE_BASE.y },
      { x: CONSTANTS.BLUE_BASE.x, y: CONSTANTS.RED_BASE.y },
      { x: CONSTANTS.RED_BASE.x, y: CONSTANTS.RED_BASE.y }
    ]
  },
  bot: {
    points: [
      { x: CONSTANTS.BLUE_BASE.x, y: CONSTANTS.BLUE_BASE.y },
      { x: CONSTANTS.RED_BASE.x, y: CONSTANTS.BLUE_BASE.y },
      { x: CONSTANTS.RED_BASE.x, y: CONSTANTS.RED_BASE.y }
    ]
  }
};

// 3. VÁCH ĐÁ CHUẨN MOBA (MÓNG NGỰA HANG RỒNG, BARON & 4 CÁNH RỪNG)
export const ROCK_WALLS = [
...
// 4. HỆ THỐNG BỤI CỎ CHIẾN THUẬT (BUSHES)
export const BUSHES = [
